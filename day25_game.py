import pygame
from pygame_common import View, Controller
...
class Plotter:
    ...
    def update(self, view, controller):
        ...
        for y in range(0,self.h):
            ...
            for x in range(0,self.w):
                ...
                if self.lines[y][x] == dc and self.lines[ny][nx] == '.':
                    ...
                    newlines[y][x] = '.'
                    self.mc += 1
        print(dc,dx,dy, view.frame_cnt//2, self.mc)
        for y in range(0,self.h):
        ...
        txt = "Move: {} {}".format(view.frame_cnt//2+1, dc)
        view.font.render_to(view.win, (20, 20), txt, (255, 255, 255))
        if self.mc == 0 and dc =='v':
            controller.animate = False
        

def init(controller, datfile):
    controller.add(Plotter(datfile))
    return controller

if __name__ == "__main__":
    # pass
    datfile = Path("day25_trial.txt").read_text().splitlines()

    # view= View(W = 400, H = 200)
    view = View()
    view.setup("Day 25")
    controller = Controller()
    init(controller, datfile).run(view) # wait